...
const { loginListener, listListeners, deleteListener, walletBalanceListeners, withdrawListener, updateProfile, getUserOrListenerDetails, listListenerTransactions, listListenerWithdrawals, requestProfileUpdate, getMissedMeetings, getPenalties, requestLeave, markAttendance, handleConnectionRequest, getListenerInboxNotifications, sendMessageToUser, getListenerMessages, submitListenerForApproval } = require("../controller/listenerController");
const validateToken = require("../middleware/validateTokenHandler");
const listenerCheck = require("../middleware/listenerToken");
const router = express.Router();

router.post("/login", loginListener);
router.get("/listlisteners", validateToken, listenerCheck, listListeners);
...
router.post("/submitForApproval", submitListenerForApproval);
