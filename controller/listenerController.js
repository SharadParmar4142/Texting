...
const uploadImageToS3 = require('../utils/uploadImage'); // utility function to upload images to S3
...
const handleConnectionRequest = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const connectionRequest2 = await prisma.connectionRequest.findUnique({
            where: { id: requestId }
        });

        if (!connectionRequest2) {
            return res.status(404).json({ message: "Connection is either invalid or unavailable." });
        }
        ...
        if (userDeviceToken) {
            io.getIo().to(userDeviceToken).emit(event, connectionRequest);
        }
...
// @desc Submit listener details for approval
// @route POST /listener/submitForApproval
// @access Public
const submitListenerForApproval = asyncHandler(async (req, res) => {
    const { name, email, image, age, language, favoriteFood, hobbies, idols, sex, about, device_token, device_token2 } = req.body;
    ...
    try {
        const pendingListener = await prisma.pendingListener.create({
            data: { name, email, image, age, language, favoriteFood, hobbies, idols, sex, about, device_token, device_token2 }
        });
        res.status(201).json({ message: "Listener details submitted for approval", pendingListener });
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
});
