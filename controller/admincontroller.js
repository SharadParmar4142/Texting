...
const registerListener = asyncHandler(async (req, res) => {
    ...
    try {
        const listener = await prisma.listener.create({
            data: { id: 1000, name, email, image, age, language, favoriteFood, hobbies, idols, sex, about, device_token,device_token2 }
        });
...
const updateListenerProfile = asyncHandler(async (req, res) => {
    const { id } = req.params;
    const { name, email, age, language, favoriteFood, hobbies, idols, sex, about, device_token, device_token2 } = req.body;
    ...
    try {
        const updatedListener = await prisma.listener.update({
            where: { id: parseInt(id) },
            data: { name, email, age, language, favoriteFood, hobbies, idols, sex, about, device_token, device_token2 }
        });

        // Log the updated listener for debugging
